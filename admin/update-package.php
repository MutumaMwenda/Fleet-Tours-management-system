...
$sql="update tbltourpackages set PackageName=?,PackageType=?,PackageLocation=?,PackagePrice=?,PackageFetures=?,PackageDetails=? where pid=?";
$query = $dbh->prepare($sql);

$query->execute([$pname,$ptype,$plocation,$pprice,$pfeatures,$pdetails,$pid]);
if ($query->execute()){
...
<html>
<head>
<title>SAT | Admin Package Update</title>
...
<?php 
$pid=intval($_GET['pid']);
$sql = "SELECT * from TblTourPackages where pid=:pid";
$query = $dbh -> prepare($sql);
...
<div class="col-sm-8">
<img src="pacakgeimages/<?php echo htmlentities($result->PackageImage);?>" width="200">&nbsp;&nbsp;&nbsp;<a href="change-image.php?imgid=<?php echo htmlentities($result->pid);?>">Change Image</a>
</div>
</div>
